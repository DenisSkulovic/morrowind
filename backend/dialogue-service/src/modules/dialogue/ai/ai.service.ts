...
interface IAiService {
    ...
    interrupt(requestId: string, provider: AiServiceEnum): Promise<boolean>;
    checkStatus(requestId: string, provider: AiServiceEnum): Promise<AiTaskStatusEnum>;
}

@Injectable()
export class AiService implements IAiService, OnModuleInit {
    private aiServices: Map<AiServiceEnum, IAiGRPCService> = new Map();
    ...
    onModuleInit() {
        this.aiServices.set(AiServiceEnum.AI_SERVICE_OPENAI_V1, this.grpcOpenAiV1Client.getService<IAiGRPCService>('AiServiceOpenAIv1'));
    }
     ...
     */
    private _getService(provider: AiServiceEnum): IAiGRPCService {
    ...
    /**
     * Checks the status of the AI request.
     * @param requestId - The requestId of the AI request to check the status of.
     * @param provider - The AI provider implementation to use.
     * @returns The status of the AI request.
     */
