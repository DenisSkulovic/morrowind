...
const dialogueOptionRiskImpact_meaning = `
    "riskImpact" is an attribute of SCALE_RISK_IMPACT_ENUM, reflecting the potential risk and impact associated with the character's chosen direction and its outcome.
    ...
    The possible values of SCALE_RISK_IMPACT_ENUM are: ${JSON.stringify(Object.values(SCALE_RISK_IMPACT_ENUM))}.
`
...
export class PromptService {
    ...
    public assembleGeneratePlayerDialogueOptionsPrompt(
        ...
        variations: number,
        mandatoryDirections: DialogueDirectionEnum[] | null = null,
        allowedDirections: DialogueDirectionEnum[] | null = null,
    ): string {
        if (allowedDirections && allowedDirections.length === 0 && mandatoryDirections && mandatoryDirections.length > 0) throw new Error(`[PromptService - assembleGeneratePlayerDialogueOptionsPrompt] If allowedDirections is empty, mandatoryDirections must also be empty.`);
        if (allowedDirections && allowedDirections.length < directionsQuantity) throw new Error(`[PromptService - assembleGeneratePlayerDialogueOptionsPrompt] directionsQuantity must be less than or equal to the number of allowed directions. Quantity: ${directionsQuantity}, allowedDirections: ${JSON.stringify(allowedDirections)}`);
        if (mandatoryDirections && mandatoryDirections.length > directionsQuantity) throw new Error(`[PromptService - assembleGeneratePlayerDialogueOptionsPrompt] directionsQuantity must be greater than or equal to the number of mandatory directions. Quantity: ${directionsQuantity}, mandatoryDirections: ${JSON.stringify(mandatoryDirections)}`);
        if (mandatoryDirections && mandatoryDirections.length > 0 && allowedDirections && allowedDirections.length > 0) {
            mandatoryDirections.forEach(direction => {
                if (!allowedDirections.includes(direction)) throw new Error(`[PromptService - assembleGeneratePlayerDialogueOptionsPrompt] Mandatory direction ${direction} is not present in allowed directions.`);
            });
        }
        return `
            Your task is to generate ${directionsQuantity} dialogue direction options, each with ${variations} variations (i.e. with different risk and impact levels) for the player to choose from to either progress or conclude the dialogue.
            For example, if quantity is set to 2 and variations to 3, you will generate 6 options total - two, two and two with same direction, but different risk and impact levels. Rely on the characters and context to generate engaging and diverse options, to give the player tools to progress (or end) the dialogue.
            ...
            Reference for dice scales (where "sides" indicates the type of dice, e.g., 20 for D20, and keys represent SCALE_RISK_IMPACT_ENUM values): ${JSON.stringify(scaleConfig)}.
            ...
            IMPORTANT: For each of the ${directionsQuantity} directions, generate ${variations} options (low, medium, high risk). The AI should determine the specific risk and impact settings based on the situation and character traits, ensuring alignment with the proposed options.

            Here are all the possible dialogue directions: ${JSON.stringify(allowedDirections || Object.values(DialogueDirectionEnum))}
            ${mandatoryDirections && mandatoryDirections.length > 0 ? `
                Here are directions that you must include: ${JSON.stringify(mandatoryDirections)}
                The rest you can choose from the allowedDirections array.
                ` : ''
            }

            Here are all the possible values of SCALE_RISK_IMPACT_ENUM: ${JSON.stringify(Object.values(SCALE_RISK_IMPACT_ENUM))}
